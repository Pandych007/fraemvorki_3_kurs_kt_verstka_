...
export class RecipesService {
  ...
  changeTimeLevel(
    newTime: 'breakfast' | 'lunch' | 'dinner',
    newLevel: 'all' | 'easy' | 'medium'
  ): void {
    this.time = newTime;
    this.level = newLevel;
    this.changeMainRecipes();
  ...
  changeMainRecipes(): void {
    this.mainRecipes = this.recipes.filter(
      (recipe: Recipe) =>
        (this.level !== 'all'
          ? recipe.difficulty.toLocaleLowerCase() === this.level
          : true) &&
        recipe.mealType
          .map((meal) => meal.toLocaleLowerCase())
          .includes(this.time)
    );
  }
  ...
  async changeSelectedRecipe(id: number): Promise<void> {
    ...
    const visitedRecipes: number[] = JSON.parse(
      localStorage.getItem('visitedRecipes') ?? '[]'
    );
    localStorage.setItem(
      'visitedRecipes',
      JSON.stringify([...visitedRecipes, id])
    );
  }
  ...
  getUnVisitedRecipes(): Recipe[] {
    ...
    return this.recipes.filter(
      (recipe: Recipe) => !visitedRecipes.includes(recipe.id)
    );
  }
}
